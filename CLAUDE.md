...
**Retired rules are force-pruned.** `mergeSettings` unions the deployed arrays with the SoT arrays, so deleting a rule from the SoT never removes it from a machine an earlier sync already wrote. `claudeRetired.ts RETIRED_PERMISSION_RULES` holds the exact strings the kit no longer ships — the 128 shell allow rules, `WebFetch`, and the 14 malformed single-backslash deny spellings — and `claudeSync.ts syncRemovals` force-prunes them on every sync. Exact strings only: a rule you added yourself outside that inventory survives. Retiring a rule therefore takes two edits, the SoT deletion and the inventory entry.
