...
| `advisorModel` | `fable` | Server-side second-opinion tool (v2.1.98+; `fable` alias needs 2.1.170): Claude consults the advisor model at decision points — before committing to an approach, on recurring errors, before declaring done — with the full conversation. Pairing rules are API-enforced: Fable main → only a Fable advisor; Opus main → Fable or Opus ≥ main version. Requires Fable 5 org access; advisor calls bill at advisor-model rates — drop per-machine in `settings.local.json` if cost bites after the launch window. |
