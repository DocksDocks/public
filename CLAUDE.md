...
The kit sets **`"model": "opus"`** and leaves `advisorModel` unset. `opus` is an alias, not a pinned id: it resolves to the newest Opus the installed Claude Code knows — Opus 5 on the Anthropic API from v2.1.219, an older Opus below that — so the default tracks releases without outrunning the SoT `minimumVersion` of 2.1.170. Pinning `claude-opus-5` directly would hard-fail every consumer under 2.1.219, which is why the SoT keeps the alias. Advisor stays an explicit per-machine choice: `./docks-kit sync claude --claude-advisor=on` writes `advisorModel: fable`; `off` and `default` delete it, and a flag-less sync removes the former kit-owned advisor key from older deployments. Advisor needs Fable org access + Claude Code ≥2.1.170. Use `./docks-kit model claude fable` or `--claude-model=fable` for a temporary Fable-main machine profile; a flag-less sync returns to Opus. `ANTHROPIC_DEFAULT_OPUS_MODEL` stays unpinned so the `opus` alias auto-tracks new Opus releases; to pin during a known-bad release, set it in `settings.local.json`.
