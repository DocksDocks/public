...
| Variable | Value | Purpose |
|----------|-------|---------|
| `effortLevel` (top-level key, NOT the env var) | `xhigh` | Valid: `low`/`medium`/`high`/`xhigh`. Both Opus 4.8 and Fable 5 default to `high`; the kit pins `xhigh` — the official recommendation for coding/agentic work on Opus 4.7/4.8 ([effort doc](https://platform.claude.com/docs/en/build-with-claude/effort)) — set 2026-07-08 alongside the `model: opus` pin. (Historical: during the Fable-5-main period the kit ran `medium`, since low effort on Fable 5 often exceeds `xhigh` on prior models.) **Deliberately the settings key, not `CLAUDE_CODE_EFFORT_LEVEL`**: the env var is priority 1 in effort resolution and overrides per-skill/per-subagent `effort:` frontmatter (added 2.1.78–2.1.80), which would kill plugin-declared effort tiering — the settings key sets the session baseline while frontmatter wins when a skill/agent is active. Trade-off: only the env var can persist `max`; the kit deliberately avoids it (`max` can overthink structured tasks). Model-transition quirk: a new model's first run may reset settings-level effort to the model default — sync re-asserts the SoT value; confirm with `/effort`. |

#### Model selection

The kit sets **`"model": "opus"` + `"advisorModel": "fable"`** in settings.json (2026-07-08): the latest Opus runs the main thread and consults a **Fable 5 advisor** at decision points — plan commits, recurring errors, pre-completion checks (Claude decides when; force one with "consult the advisor"). This pairing is deliberate over Fable-as-main: the advisor rule is "advisor ≥ main" so Opus main + Fable advisor is valid, it keeps Fable usage to decision points (Fable is $10/$50 vs Opus's $5/$25 per MTok and draws subscription credits faster), and it dodges the open Fable-main advisor bug ([#67609](https://github.com/anthropics/claude-code/issues/67609): advisor returns `unavailable` above ~100K-token transcripts when Fable is the *main* model — as advisor it reads the transcript fresh each call). Fable-as-advisor needs org access + Claude Code ≥2.1.170; without access the advisor simply doesn't attach. To run Fable as main on one machine anyway, set `"model": "fable"` (or `best`) in `~/.claude/settings.local.json`. `ANTHROPIC_DEFAULT_OPUS_MODEL` stays unpinned so `opus` auto-tracks new Opus releases; to pin during a known-bad release, set it in `settings.local.json`. Standalone deployed-model switches: `./docks-kit model claude <value>`.
...
| Key | Value | Notes |
|-----|-------|-------|
| `model` | `opus` | Latest-Opus alias pin. See § Model selection for the Opus-main + Fable-advisor rationale and per-machine overrides. |
| `advisorModel` | `fable` | Fable 5 advisor consulted at decision points (server-side advisor tool; experimental). Needs Fable org access + ≥2.1.170 — without access the advisor silently doesn't attach. Re-added to the SoT 2026-07-08 (and un-listed from the `removed` manifest, which had pruned it since 2026-06-11). |
