...
| Variable | Value | Purpose |
|----------|-------|---------|
| `effortLevel` (top-level key, NOT the env var) | `high` | Valid persisted values: `low`/`medium`/`high`/`xhigh`. Opus 5.5's Claude API default effort is `medium`, so the kit pins `high` deliberately, one step above the model default, for agentic coding; `--claude-effort=<level>` is the per-machine override and `default` restores `high`. **Deliberately the settings key, not `CLAUDE_CODE_EFFORT_LEVEL`**: the env var is priority 1 in effort resolution and overrides per-skill/per-subagent `effort:` frontmatter (added 2.1.78–2.1.80), which would kill plugin-declared effort tiering — the settings key sets the session baseline while frontmatter wins when a skill/agent is active. `max` is session-only through `/effort`, not part of the persisted settings enum. Model-transition quirk: a new model's first run may reset settings-level effort to the model default — sync re-asserts the SoT value; confirm with `/effort`. Claude Code 2.1.280 also stops an effort level saved before `/effort` became per-model from applying to a newly released model such as Opus 5.5. |

#### Model selection

The kit sets **`"model": "opus"`** and leaves `advisorModel` unset. `opus` is an alias, not a pinned id: the SoT `minimumVersion` of 2.1.280 ensures Claude Code resolves it to Opus 5.5, the default Opus from that release, instead of capping at Opus 5 under the former 2.1.219 floor. Keeping the alias tracks the next Opus release without a kit change and stays portable across providers that carry a different newest Opus. This floor also subsumes Fable 5.1's Claude Code ≥2.1.257 requirement. Advisor stays an explicit per-machine choice: `./docks-kit sync claude --claude-advisor=on` writes `advisorModel: opus`; `off` and `default` delete it, and a flag-less sync removes the former kit-owned advisor key from older deployments. Use `./docks-kit model claude opus` or `--claude-model=opus` for a per-machine main-model override; `default` restores the SoT alias.
...
| Key | Value | Notes |
|-----|-------|-------|
| `model` | `opus` | Opus alias pin, resolving to Opus 5.5 at the `minimumVersion` floor. Use the model modifier for a per-machine override. |
| `advisorModel` | absent (off) | Advisor is opt-in via `--claude-advisor=on` (`opus`). The removed manifest prunes the formerly kit-owned key on flag-less sync; explicit `off`/`default` also delete it. |
| `effortLevel` | `high` | See § Thinking & reasoning — one step above Opus 5.5's `medium` API default, pinned as a settings key so plugin `effort:` frontmatter still applies. |
...
| `alwaysThinkingEnabled` | `true` | Tells Claude Code to opt into adaptive thinking on every turn. On 4.7, adaptive thinking is off by default at the API layer and must be explicitly enabled — this flag handles that. Moot on Opus 5.5 and Fable 5.1, where adaptive thinking is always on; still load-bearing on an older Opus fallback. |
...
| `minimumVersion` | `2.1.280` | Floor for auto-updates and `claude update` — a stale install upgrades to ≥2.1.280 on next launch, so the `opus` alias resolves to Opus 5.5 instead of capping at Opus 5. It subsumes every older requirement the kit relies on (Opus 5.5 needs 2.1.280; Fable 5.1 — incl. as an advisor choice — needs 2.1.257; Opus 5 needs 2.1.219; `skillListingBudgetFraction` needs 2.1.129). Distinct from the managed-only `requiredMinimumVersion`. |
...
| `--claude-advisor=<on\|off\|default>` | `on` writes `advisorModel: opus`; `off`/`default` delete it | Opt one machine into the Opus advisor; the SoT and flag-less sync keep advisor off |
...
Two situational plugins are kept out of the SoT entirely and opted in per machine. Neither key is in `enabledPlugins`, so a flag-less sync installs, loads, and enables neither (an absent plugin is simply not installed). Unlike `--claude-compact-window`/`--claude-permissive`, the opt-in is **sticky**: once `--claude-plugin=<name>` installs and enables a plugin it stays until you run `--prune` — a later flag-less sync won't revert it (the SoT has no key to reassert against). The flag is repeatable and accepts comma-separated names (`--claude-plugin=supabase,n8n`); unknown names exit 2. Implemented by `claudeOptionalPlugins.ts syncOptionalPlugins, sticky opt-in install`, which runs right after `claudePluginPasses.ts syncPlugins, seven-pass reconcile`.
