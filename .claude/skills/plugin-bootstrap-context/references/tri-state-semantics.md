...
- The "Globally disabled" state for a `false`-keyed plugin is **enforced by Pass 7**, not by the settings merge alone. Pass 2's `claude plugin install` enables the plugin at user scope as a side effect, so `claude::_plugins_reassert_enabled_state` must re-write the SoT `false` afterward. This only affects marketplace-installed plugins (e.g. `n8n-mcp-skills`); built-in `claude-plugins-official` plugins like `supabase` take a different install path and keep their merged value.
