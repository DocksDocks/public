...
`<slug>` MUST be the FIRST positional argument. The `-a/--agent` flag is variadic and consumes everything after it as agent names. (lib/skills.sh:74-77)
...
Failure mode is silent: the command exits 0, no error is printed, `~/.agents/skills/<name>/` is never created. On the next sync pass, `[[ -d "$SKILLS_DIR/$basename" ]]` (lib/skills.sh:66) returns false, so the add is retried — and fails again silently every time.

## Regression History

This trap was hit once before (documented in `docs/plans/finished/2026-05-14-skills-cli-storage-model-doc-sync.md`). The comment at lib/skills.sh:74-77 documents the reason:
...
## Correct Invocation (lib/skills.sh:78)
...
## Corresponding Remove Invocation (lib/skills.sh:211)
