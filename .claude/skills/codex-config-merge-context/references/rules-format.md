...
`~/.codex/rules/default.rules` (user-learned approvals) is NEVER overwritten by the kit. Only kit-managed `*.rules` files from `SoT/.codex/rules/` are deployed. (lib/codex.sh:104-121)
...
2. `lib/codex.sh:codex::sync_rules` automatically deploys any `*.rules` file in `SoT/.codex/rules/` (lib/codex.sh:118)
3. Backup of the old `<name>.rules` is written to `<name>.rules.bak` before overwrite (lib/codex.sh:115)
