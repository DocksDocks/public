...
<constraint>
Never construct SoT paths from `$PWD` — always use `$REPO_DIR/SoT/…` where `REPO_DIR` is set from `$(cd "$(dirname "$0")" && pwd)` (sync.sh (REPO_DIR assignment)).
</constraint>

<constraint>
Every new sync step that can legitimately fail non-fatally MUST add `|| true` or `|| warn "…"` — `set -euo pipefail` is active in all five bash files (sync.sh (set -euo pipefail)).
</constraint>

<constraint>
Never re-initialize flag variables (`DRY_RUN`, `FORCE`, `REMOVE_PLUGINS`, `SKIP_OPTIONAL_BOOTSTRAP`, `SYNC_CLAUDE`, `SYNC_CODEX`, `SYNC_AGENTS`) inside `lib/*.sh` — they are initialized via `${VAR:-0}` in common.sh (flag-var init block) and may be pre-set as environment variables.
</constraint>

<constraint>
Every sync step must open with `if [[ "$DRY_RUN" -eq 1 ]]; then echo "[dry-run] <what would happen>"; return; fi` — the `[dry-run]` prefix is canonical (claude::sync_scripts dry-run pattern). `--force` (settings layer) and `--remove-plugins` (plugin+skills layer) are orthogonal — never conflate their scopes.
</constraint>
...
5. Verify the SoT-presence guard pattern: each tool lib is sourced inside `[[ -d "$REPO_DIR/SoT/.<tool>" ]]` — never at top-level (sync.sh (SoT-presence dispatch blocks)).
6. Add `declare -F` guard for any summary/next_steps functions — keeps them no-op when the tool was not synced (sync.sh (summary/next_steps declare-F guards)).
...
SoT-presence guard + lazy lib source (sync.sh (Claude dispatch block)):
```bash
...
Default-all-three when no target flag (common::parse_args (default-all-three)):
```bash
...
`declare -F` guard for summary (sync.sh (summary declare-F guard)):
```bash
if declare -F claude::summary >/dev/null 2>&1; then claude::summary; fi
```

Unknown-flag error (common::parse_args (unknown-flag exit 2)): `err "Unknown arg: $arg"; exit 2`
...
3. Confirm sync.sh (SoT-presence dispatch blocks) pattern before describing SoT-presence guard behavior.
4. When citing skill paths, confirm `.claude/skills/sync-orchestration-context/` exists on disk.

## Success Criteria

- Flag change correctly sets/unsets the documented variable (verified by `common::parse_args` contract).
