...
<constraint>
The Codex marketplace `unique_by(.name)` dedup concatenates `$user.plugins + $repo.plugins`, then reverse-unique-reverse keeps the last-in-original occurrence on a `.name` collision — so the SoT/repo entry (concatenated last) wins; user-only plugins with no colliding `.name` survive additively (`codex::sync_marketplace`).
</constraint>

## Workflow

1. Read `.claude/skills/plugin-bootstrap-context/SKILL.md` for the seven-pass structure and tri-state semantics.
2. For detailed pass logic or Codex marketplace dedup, read `.claude/skills/plugin-bootstrap-context/references/seven-pass-flow.md`.
...
Tri-state examples — every entry the SoT ships is `true`; `false` is a supported value the kit doesn't currently use:
```json
"docks@docks": true,
...
- `.claude/skills/plugin-bootstrap-context/SKILL.md` — seven-pass structure, tri-state semantics, Codex marketplace merge
- `.claude/skills/plugin-bootstrap-context/references/seven-pass-flow.md` — annotated pass-by-pass walkthrough, failure counters
...
3. Confirm the `true`-valued example entries match `SoT/.claude/settings.json`; the SoT ships no `false` entry today, so treat any `false` example as illustrative of the supported value, not a shipped one.
