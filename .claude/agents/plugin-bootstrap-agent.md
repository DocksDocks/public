...
description: Use when modifying `claude::sync_plugins` (six-pass reconciler), the `enabledPlugins` tri-state semantics, `extraKnownMarketplaces` entries, `codex::sync_marketplace`, `codex::bootstrap_marketplace`, or the `claude-plugins-official` removal protection. Not for `~/.claude/settings.json` key merge (use `settings-json-agent`) or universal skill install (use `skills-bootstrap-agent`).
...
<constraint>
Use `jq -e --arg n "$plugin_id" '.enabledPlugins | has($n)'` (NOT truthiness) to test whether a plugin should be kept on `--remove-plugins` — `false`-valued keys must pass this test to be preserved (`lib/claude.sh:198`).
</constraint>

<constraint>
Never remove `claude-plugins-official` marketplace — it is a protected built-in; the removal loop explicitly skips it (`lib/claude.sh:212`: `[[ "$mp_name" == "claude-plugins-official" ]] && continue`).
</constraint>
...
6. For removal of a plugin: remove from `enabledPlugins` entirely (not set to `false`) so `--remove-plugins` uninstalls it; the `has($n)` test at `lib/claude.sh:198` gates this.
...
Marketplace add pre-check — key presence only (`lib/claude.sh:157-168`):
```bash
jq -e --arg n "$mp_name" '.[$n]' "$known_marketplaces" >/dev/null 2>&1
```

Removal guard using `has()` not truthiness (`lib/claude.sh:198`):
```bash
if ! jq -e --arg n "$plugin_id" '.enabledPlugins | has($n)' "$repo_settings" >/dev/null 2>&1; then
```

Protected built-in guard (`lib/claude.sh:212`):
```bash
...
- Hand off to `skill-author-agent` when `plugin-bootstrap-context` SKILL.md needs updating

## Anti-Hallucination Checks

1. Before citing `lib/claude.sh:198`, read that line and confirm it uses `has($n)`, not a truthiness test.
2. Before citing `lib/claude.sh:212`, read that line and confirm the `claude-plugins-official` guard string exactly.
...
- The marketplace pre-check at `lib/claude.sh:159` tests key presence, not URL validity. A marketplace added with a bad URL stays "present" — recovery requires manual `claude plugin marketplace remove <name>` then re-sync.
