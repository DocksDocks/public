...
Tri-state examples (the `enabledPlugins` block in `SoT/.claude/settings.json`):
```json
"docks@docks": true,
...
3. Confirm the `enabledPlugins` block values in `SoT/.claude/settings.json` match the tri-state table before describing semantics.
