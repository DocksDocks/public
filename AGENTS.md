...
- Claude's deployed SoT defaults are `model: opus` and `effortLevel: high`; `advisorModel` is deliberately absent/off. `--claude-advisor=on` is the per-machine opt-in and writes `advisorModel: fable` after the settings merge. `opus` is the alias, not a pinned id: it resolves to the newest Opus the installed Claude Code knows (Opus 5 on the Anthropic API from v2.1.219), so the SoT default never outruns `minimumVersion` — pinning `claude-opus-5` would break consumers below 2.1.219.
