...
- **Targeted syncs.** `sync.sh` accepts per-CLI target flags: `--claude`, `--codex`, and `--agents`. Use the narrowest target that matches the SoT change (for example, `./sync.sh --codex` for Codex-only config edits); target flags can be combined with `--dry-run`, `--no-rtk`, `--force`, `--remove-plugins`, and the Claude-only deploy-time modifiers `--fable` / `--permissive` (see `CLAUDE.md` § Deploy-time modifiers).
