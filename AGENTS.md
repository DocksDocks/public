...
- `SoT/.omp/config.yml` sets `compaction.thresholdTokens` to `-1`, which is omp's schema default sentinel selecting reserve-based behavior. The reserve-based default resolves the trigger to `contextWindow` minus `max(floor(contextWindow * 0.15), 16384)`, which is 231,200 on the 272,000-token Codex window and 850,000 on the 1,000,000-token Anthropic window. The kit previously pinned `231200`. That value is exactly the default for a 272,000-token window, so it was a no-op on the Codex lane and forced the Anthropic lane to compact at 23.1% of its window instead of 85%. The key ships as `-1` rather than being deleted because `ompYaml.ts mergeOmpConfig` is additive: `mergeMappings, deployed-key retention loop` returns every deployed key absent from SoT to the merged output, and the only drop predicate is `dropFallbackWildcard`, scoped to `retry.fallbackChains`. A deletion would leave the stale `231200` in every already-deployed `~/.omp/agent/config.yml`. `cli/docs/omp-context.md` (topic `omp-context`) records the full derivation, the per-lane table, and the measured evidence. Update that topic in the same commit as any compaction-setting change, in the same way the existing bullet requires for `omp-models` and role changes.
...
- **Pin, never float.** Every kit-driven install of third-party software is pinned to a `SoT/toolchain.json` `verified` version — no `@latest` npm/bun installs (Shai-Hulud-class worm surface), no mutable action tags in workflows (commit SHAs only), installer scripts fetched from version tags where upstream supports it. New third-party install surface ⇒ manifest pin first. The rule also covers a transitive dependency whose declared range is a caret or tilde over a prerelease: `^4.0.0-rc.109` admits every later prerelease of the same version tuple, and a published install resolves that range fresh because `bun.lock` is not packaged, so a runtime pinned in `package.json` meets a newer sibling and the process fails at startup. The fix is an exact root pin plus both guards: the offline `cli/test/unit/dependencyPins.test.ts` and the clean-room `bun run smoke:package`. Moving the Effect release is therefore one lockstep commit: set `effect`, `@effect/platform-bun`, `@effect/platform-node-shared`, and `@effect/vitest` to the same version together, then run `bun run smoke:package`; the guard reports `unsatisfied` while one of them lags. Details: the `toolchain-context` skill and `cli/docs/toolchain.md`.
