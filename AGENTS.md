...
| `cli/build-binaries.sh` | Compiles the six release binaries. It checksums the six known artifact names rather than globbing `docks-kit-*`, so a packed `docks-kit-<version>.tgz` is never attested as a release binary. It stamps `cli/dist/VERSION` and warns when a partial rebuild retains binaries from another version, because `SHA256SUMS` then spans more than one version. It never deletes a binary, for the same reason the launchers do not. |
