...
- **The bash engine is feature-frozen** (decision 2026-07-08): new engine capabilities go into the TypeScript EngineNative port behind `cli/src/engine.ts` (see the `windows-support` plan); `lib/*.sh` accepts bug fixes only. The bash engine remains the no-Bun escape hatch until EngineNative reaches dry-run parity and becomes the default.
