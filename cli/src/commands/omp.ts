...
const loadFreeCatalog = (omp: string): Effect.Effect<ReadonlyArray<CatalogModel>> =>
  Effect.gen(function* () {
    ...
    if (free.length === 0) {
      return yield* bail(
        "'omp models --json' listed no free models; verify the omp login, then retry",
      );
    }
    return free;
  });
...
const pickLevels = (catalogModel: CatalogModel) =>
  Effect.gen(function* () {
    ...
    if (plan.kind === "none") {
      yield* Console.error(
        `${catalogModel.name} publishes no thinking levels; the model default applies`,
      );
      ...
      choices: [
        {
          title: "Yes",
          value: true,
          description: `every role runs at ${plan.highest}, the highest this model offers`,
        },
    ...
    const ladderChoices = plan.levels.map((level) => ({
      ...
      description:
        level === plan.highest
          ? "highest this model offers"
          : level === plan.levels[0]
            ? "lowest this model offers"
            : "",
    }));
...
export const ompCommand = Command.make("omp", { model, pick, args }, (config) =>
  Effect.gen(function* () {
    ...
    if (Option.isSome(config.model)) {
      ...
      yield* Effect.sync(() =>
        writeOmpSessionModel(home, {
          selector: resolved.model.selector,
          thinking,
          advisorThinking,
        }),
      );
    ...
    const session: OmpSessionModel =
      (yield* Effect.sync(() => readOmpSessionModel(home))) ?? DEFAULT_OMP_SESSION_MODEL;
    ...
    const level =
      typeof session.thinking === "string" && session.thinking.trim() !== ""
        ? session.thinking
        : undefined;
    const advisor =
      typeof session.advisorThinking === "string" && session.advisorThinking.trim() !== ""
        ...
        : undefined;
    const levelText =
      level === undefined ? "the model default thinking level" : `${level} thinking`;
