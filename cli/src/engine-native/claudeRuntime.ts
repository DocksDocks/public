...
function countString(value: Json, expected: string): number {
  ...
  if (Array.isArray(value))
    return value.reduce<number>((total, item) => total + countString(item, expected), 0);
  if (!isObject(value)) return 0;
  return Object.values(value).reduce<number>(
    (total, item) => total + countString(item, expected),
    0,
  );
}

function hooksObject(template: Json): { [key: string]: Json } {
  if (!isObject(template) || !isObject(template["hooks"]))
    throw new Error("Claude settings hooks object is missing");
...
function validateTemplate(template: Json): void {
  ...
  if (sessionStart["command"] !== BUN_SENTINEL)
    throw new Error("SessionStart Bun sentinel location is invalid");
  if (notification["command"] !== BUN_SENTINEL)
    throw new Error("Notification Bun sentinel location is invalid");
  ...
  if (hooks["Stop"] !== undefined)
    throw new Error("hooks.Stop must be absent from the Claude settings template");
  if (
    !isObject(template) ||
    !isObject(template["statusLine"]) ||
    template["statusLine"]["command"] !== STATUSLINE_SENTINEL
  ) {
  ...
  for (const [label, sentinel, expected] of counts) {
    if (countString(template, sentinel) !== expected)
      throw new Error(`${label} sentinel residue/cardinality is invalid`);
  }
}
...
export function materializeClaudeSettings(
  ...
  if (runtime === undefined) {
    ...
    if (!isObject(result) || !isObject(result["statusLine"]))
      throw new Error("Claude statusLine object is missing");
  ...
  for (const sentinel of [
    ...
    if (countString(result, sentinel) !== 0)
      throw new Error(`Claude settings sentinel residue: ${sentinel}`);
  }
  return result;
}
