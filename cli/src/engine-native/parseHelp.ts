...
export function printUsage(ctx: Ctx, knownPlugins: ReadonlyArray<string>): void {
  ...
  echo(
    "  --verbose         also print no-op confirmations (already in sync, up to date, left as-is)",
  );
  echo("");
  echo(
    "Deploy-time modifiers (deployed config only; SoT untouched; a later flag-less sync reverts)",
  );
  echo(
