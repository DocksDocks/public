...
type BunInstallResult =
  ...
  | {
      readonly ok: false;
      readonly reason: "download-failed" | "installer-failed";
      readonly detail: string;
    };

function processFailure(result: AsyncProcessResult): string {
  const details = [result.error?.message, result.stderr.trim()].filter(
...
async function runBunBootstrap(ctx: Ctx, services: EngineServices): Promise<BunRuntimeState> {
  ...
  if (installed === "") {
    services.logger.warn(
      "Bun install failed. Install manually from https://bun.sh/docs/installation, then re-run sync.",
    );
    return { kind: "deferred", reason: "install-failed" };
  }
