...
import { syncClaudeAdvisor, syncClaudeEffort, syncClaudeModel } from "./claudeSettingsModifiers";
import {
  claudeRuntimePaths,
...
export async function claudeSync(ctx: Ctx): Promise<ClaudeRuntimeState> {
  ...
  const runtime: ClaudeRuntimeState =
    bun.kind === "ready"
...
function syncClaudeRuntime(ctx: Ctx, runtime: ClaudeRuntimeState): void {
  ...
  if (runtime.kind === "deferred") {
    warn(
      "Bun unavailable — Claude statusline/hooks migration deferred; install Bun, then re-run sync claude",
    );
    return;
  }
  if (ctx.dryRun) {
  ...
  if (
    writeBytesIfChanged(
      p(ctx.home, ".claude", "notification.mp3"),
      payloadBytes("notification.mp3"),
    )
  )
    changed = true;
  if (changed) {
...
function syncClaudeJson(ctx: Ctx): void {
  ...
  if (ctx.dryRun) {
    ...
    if (haveMcp)
      echo("[dry-run] merge mcpServers from SoT/.claude/mcp-servers.json into ~/.claude.json");
    return;
  }
  ...
  const applyFilter = (doc: JsonObject): void => {
    ...
    if (haveMcp) {
      ...
      doc["mcpServers"] = deepMerge(
        isObject(doc["mcpServers"]) ? doc["mcpServers"] : {},
        sotServers,
      );
    }
  };
  ...
  if (changed) {
    ...
    verbose(`~/.claude.json already in sync (showTurnDuration${haveMcp ? ", mcpServers" : ""})`);
}
...
async function syncConnectorEnv(ctx: Ctx): Promise<void> {
  ...
  switch (setting.kind) {
    ...
    case "command": {
      const existing = await spawnProcess(setting.probe.command, setting.probe.args, {
        stdio: "ignore",
      });
      if (existing.error === undefined && existing.exitCode === 0) {
        if (ctx.dryRun) echo(`[dry-run] ${name} already in ${setting.location} — would skip`);
        else
          verbose(`claude.ai connectors: ${name} already set in ${setting.location} (left as-is)`);
        return;
      }
      ...
      const applied = await spawnProcess(setting.apply.command, setting.apply.args, {
        stdio: "ignore",
      });
      if (applied.error === undefined && applied.exitCode === 0) {
        change(
          `claude.ai connectors disabled via ${setting.apply.command} (open a new terminal to apply)`,
        );
        ctx.nextStepTriggers.claudeRestart = true;
      } else {
...
export function claudeSummary(ctx: Ctx, runtime: ClaudeRuntimeState): void {
  ...
  if (!ctx.dryRun) {
    ...
    if (ctx.services.deps.probe("claude").state === "present") {
      ...
      const count =
        installed !== undefined && isObject(installed) && isObject(installed["plugins"])
          ? Object.keys(installed["plugins"]).length
          : 0;
