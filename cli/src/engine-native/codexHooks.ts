...
function isRetiredImportedSessionStartHook(handler: Json, codexDir: string): boolean {
  ...
  return (
    command === RETIRED_CONTEXT_HOOK ||
    command === RETIRED_SKILLS_HOOK ||
    RETIRED_CONFIG_HOOKS.has(command) ||
    command === retiredConnectorHook
  );
}

function withoutRetiredImportedHooks(
  doc: { [key: string]: Json },
  codexDir: string,
): { readonly value: Json; readonly removed: number } {
...
export function removeRetiredImportedHooks(ctx: Ctx, codexDir: string): void {
  ...
  if (ctx.dryRun) {
    echo(
      `[dry-run] remove ${result.removed} retired imported docks-kit SessionStart hook(s) from ${hooksFile}`,
    );
    return;
  }
  ...
  change(
    `Codex: removed ${result.removed} retired imported docks-kit SessionStart hook(s) (backup at hooks.json.bak)`,
  );
