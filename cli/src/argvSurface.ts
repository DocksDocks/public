...
const takesValueInAnySurface = (name: string, surfaces: ArgvSurfaces): boolean => {
  ...
  if (globalName !== undefined && surfaces.globalSurface.valueFlags.includes(globalName)) return true
  for (const surface of surfaces.commandSurfaces.values()) {
...
export const spliceOmpBoundary = (
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (token === name && takesValueInAnySurface(name, surfaces)) {
      const next = args[index + 1]
      if (next === "--") return args
      if (next !== undefined && !(next.startsWith("-") && declaredInAnySurface(flagNameOf(next), surfaces))) {
        index++
        continue
      }
    }
  ...
  for (let index = ompIndex + 1; index < args.length; index++) {
    ...
    if (takesValue && !token.includes("=")) {
      ...
      if (
        next !== undefined &&
        !(next.startsWith("-") && declaredFlagName(flagNameOf(next), surface, surfaces) !== undefined)
      ) {
        index++
      }
    }
