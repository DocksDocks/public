...
} from "../../src/engine-native/parseArgs";
...
describe("arg and effort contract", () => {
  ...
  it("accepts known plugins and rejects unknown ones with code 2", () => {
    ...
    const code = exitCode(() =>
      parseClaudePlugin("bogus", (message) => void messages.push(message)),
    );
  ...
  it("validates embedded defaults with tool-specific messages", () => {
    ...
    expect(() => validateEffortDefault("claude", "")).toThrow(
      "Embedded SoT Claude effortLevel is missing",
    );
  ...
  it("selects the legacy targets for empty input when non-interactive", () => {
    ...
    expect([ctx.syncClaude, ctx.syncCodex, ctx.syncAgents, ctx.syncOmp]).toEqual([
      true,
      true,
      true,
      false,
  ...
  it("rejects unknown positionals and renamed flags with code 2", () => {
    for (const args of [
      ...
      expect(
        exitCode(() => parseArgs(makeCtx(home), args)),
        args.join(" "),
      ).toBe(2);
    }
  });
