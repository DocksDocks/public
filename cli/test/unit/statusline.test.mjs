...
describe("statusline native formatter", () => {
  it("renders the pinned full ANSI layout from native fields", () => {
    expect(
      formatStatusline(fixture("full-input.json"), {
      ...
      `${MODEL}${PIPE}${FOLDER}${DOT}${BRANCH}${PIPE}${CONTEXT}${DIM_45_83}${PIPE}${FIVE_HOUR}${DIM_1H}${DOT}${SEVEN_DAY}${DIM_2D}`,
    );
  });
  ...
  it("floors a non-thousand compact cap and ignores a sub-1000 cap", () => {
    ...
    const capped = formatStatusline(input, {
      env: { CLAUDE_CODE_AUTO_COMPACT_WINDOW: "83001" },
      branch: "",
      cwd: "/tmp",
      nowMs: NOW_MS,
    ...
    const sevenOnly = formatStatusline(
      {
        ...
        rate_limits: {
          ...
          seven_day: { used_percentage: 100, resets_at: -1 },
        },
      },
      { branch: "", cwd: "/tmp", env: {}, nowMs: NOW_MS },
    );
...
describe("statusline program seam", () => {
  it("falls back from symbolic branch to detached HEAD without cache state", async () => {
    ...
    expect(writes).toEqual([
      `${plainPrefix("Test", "repo")}${DOT}\x1b[1m\x1b[38;2;192;103;222mabc123\x1b[22m\x1b[0m\n`,
    ]);
  });

  it.each(["not json", "[]", "null", "42"])(
    "returns silent success for invalid stdin %j",
    async (raw) => {
      const writes = [];
      expect(
        await main({
  ...
  it("direct-runs with one newline and empty stderr", () => {
    ...
    expect(result.stdout).toBe(
      `${plainPrefix("Sonnet 4.6", "repository")}${PIPE}\x1b[38;2;130;160;230mctx 25%\x1b[0m \x1b[2m\x1b[38;2;156;162;175m(50k/200k)\x1b[0m\n`,
    );
  });
});
