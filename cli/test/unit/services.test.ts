...
describe("engine service layers", () => {
  ...
  it("PlatformTest exposes the raw platform and normalized name", () => {
    ...
    expect(Effect.runSync(Effect.provide(program, PlatformTest("linux")))).toEqual([
      "linux",
      "linux",
    ]);
    expect(Effect.runSync(Effect.provide(program, PlatformTest("darwin")))).toEqual([
      "darwin",
      "darwin",
    ]);
  });
  ...
  it("DependencyManagerTest: a stub manager drives missing-tool branching", () => {
    ...
    const program = Effect.gen(function* () {
      ...
      if (deps.probe("git").state === "missing")
        deps.warnMissing("git", makeEngineServices().logger);
      return deps.spec("git").installHint();
    });
    expect(Effect.runSync(Effect.provide(program, DependencyManagerTest(stub)))).toBe(
      "install git",
    );
