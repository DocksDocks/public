...
describe("removed workflow configuration surface", () => {
  it(
    "rejects the former workflow model registry selector",
    () => {
      for (const args of [
        ["models", "workflow"],
        ...
        expect(`${result.stdout}\n${result.stderr}`, args.join(" ")).not.toContain(
  ...
  it(
    "rejects former root overrides without writing prompt files",
    () => {
  ...
  it(
    "rejects the former native workflow command",
    () => {
