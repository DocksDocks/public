...
describe("omp YAML merge contract", () => {
  ...
  it("lets SoT win on scalar and sequence conflicts", () => {
    ...
    expect(parse(mergeOmpConfig("models:\n  - kit/model\n", "models:\n  - user/a\n  - user/b\n"))).toEqual({
      models: ["kit/model"]
    })
  })
  ...
  it("drops slash keys under retry.fallbackChains in config only", () => {
    ...
    expect(parse(mergeOmpModels(sot, deployed))).toEqual({
      retry: {
        fallbackChains: { coding: ["kit/model"], reviewer: ["user/reviewer"], "provider/*": ["user/wildcard"] }
      }
    })
  })

  it("keeps slash keys outside retry.fallbackChains", () => {
    const merged = mergeOmpConfig("modelRoles:\n  coding: kit/model\n", 'modelRoles:\n  "provider/*": user/model\n')

    expect(parse(merged)).toEqual({ modelRoles: { coding: "kit/model", "provider/*": "user/model" } })
  })

  it("names the file in invalid-YAML diagnostics", () => {
    expect(() => mergeOmpConfig("value: 1\n", "[unterminated\n")).toThrow(/Invalid deployed omp config\.yml YAML:/)
    expect(() => mergeOmpModels("value: 1\n", "[unterminated\n")).toThrow(/Invalid deployed omp models\.yml YAML:/)
    expect(() => mergeOmpConfig("[unterminated\n", "value: 1\n")).toThrow(/Invalid SoT omp config\.yml YAML:/)
  })
  ...
  it("rejects non-mapping roots with the file name", () => {
    ...
    expect(() => mergeOmpConfig("value: 1\n", "42\n")).toThrow(/Deployed omp config\.yml YAML root must be a mapping/)
    expect(() => mergeOmpConfig("- a\n", "value: 1\n")).toThrow(/SoT omp config\.yml YAML root must be a mapping/)
    expect(() => mergeOmpModels("value: 1\n", "- a\n")).toThrow(/Deployed omp models\.yml YAML root must be a mapping/)
  })
