...
): OmpPaths => ompPaths({ home, env, platform });
...
describe("omp path resolution", () => {
  ...
  it("honors PI_CODING_AGENT_DIR for the default profile and ignores it under a named profile", () => {
    ...
    expect(paths({ PI_CODING_AGENT_DIR: override, OMP_PROFILE: "work" }).agentDir).toBe(
      `${home}/.omp/profiles/work/agent`,
    );
  });

  it("resolves a relative PI_CODING_AGENT_DIR against the working directory", () => {
    expect(paths({ PI_CODING_AGENT_DIR: "rel-agent" }).agentDir).toBe(
      resolve(process.cwd(), "rel-agent"),
    );
  });
  ...
  it("disables XDG while a PI_CODING_AGENT_DIR override is active", () => {
    ...
    expect(paths({ XDG_DATA_HOME: dataHome, PI_CODING_AGENT_DIR: override }).dataRoot).toBe(
      `${home}/.omp`,
    );
  });
