...
function declaredFiles(skillFile: string): ReadonlyArray<SourceFile> {
  ...
  const parsed = parse(frontmatter[1]) as {
    metadata?: { source_files?: ReadonlyArray<SourceFile> };
  };
  ...
  it("names a file that exists and ends inside it", () => {
    ...
    for (const skill of skills) {
      ...
      for (const declared of declaredFiles(skill)) {
        ...
        if (range === null) {
          findings.push(
            `${name} declares ${declared.path} lines "${declared.lines}", not "<start>-<end>"`,
          );
          continue;
        }
        ...
        if (end > real) {
          findings.push(
            `${name} declares ${declared.path} lines ${start}-${end}; the file is ${real} lines`,
          );
        }
      }
    }
