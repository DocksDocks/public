...
function executor(state: ProbeState, home: string): ProbeExecutor {
  ...
  const custom = customRoot === undefined || customRoot === "" ? fallback : `${customRoot}/bin/bun`;
  return {
...
beforeEach(async () => {
  ...
  mocks.spawnProcess
    .mockReset()
    .mockResolvedValue({ error: undefined, exitCode: 0, stdout: "", stderr: "" });
...
describe("per-run Bun bootstrap", () => {
  ...
  it("predicts the pinned POSIX path in dry-run without spawning or removing", async () => {
    ...
    expect(test.lines.join("")).toContain(
      `[dry-run] install Bun ${VERIFIED_BUN} (kit-verified) -> /custom bun/bin/bun`,
    );
    expect(mocks.spawnProcess).not.toHaveBeenCalled();
  ...
  it("predicts the pinned Windows path in dry-run without spawning or removing", async () => {
    ...
    expect(expectReady(await bunBootstrap(test.ctx, test.services))).toBe(
      "C:/custom bun/bin/bun.exe",
    );
    expect(test.lines.join("")).toContain(
  ...
  it("defers a POSIX dry-run when curl cannot satisfy the planned bootstrap", async () => {
    ...
    expect(await bunBootstrap(test.ctx, test.services)).toEqual({
      kind: "deferred",
      reason: "missing-curl",
    });
    expect(test.lines.join("")).toContain(
    ...
    async ({ platformId, hostId, installerName, expectedExecutable }) => {
      ...
      mocks.spawnProcess.mockImplementation(async (cmd: string, args: ReadonlyArray<string>) => {
        if (cmd === "curl") {
          ...
          if (
            target === undefined ||
            (!target.startsWith(`${testRoot}/`) && !target.startsWith(`${testRoot}${sep}`))
          ) {
            throw new Error(
              `curl stub expected an -o target under ${testRoot}, received ${String(target)}`,
            );
          }
          downloadedTo = target;
  ...
  it("reports transport diagnostics when the Bun installer download fails", async () => {
    ...
    expect(test.lines.join("")).toContain(
      "Bun installer download failed (curl: (22) server returned 503)",
    );
  });
  ...
  it("reports installer diagnostics when Bash rejects the downloaded installer", async () => {
    ...
    mocks.spawnProcess
      ...
      .mockResolvedValueOnce({
        ...
        exitCode: 2,
        stdout: "",
        stderr: "install: unsupported platform",
      });
