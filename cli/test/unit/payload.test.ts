...
describe("generated SoT payload", () => {
  ...
  it("embeds the lean global Claude and Codex inventory", () => {
    ...
    expect(codexPluginIds).toEqual(["docks@docks", "plan-lifecycle@docks"]);
    expect(marketplacePluginIds).toEqual(codexPluginIds);
    expect(mcp.mcpServers).toEqual({});
    expect(slugs).toEqual([]);
    expect(claude).not.toMatch(
      /^## (Project Skills|Project Agents|Picking the right models for workflows and subagents|Agentic Engineering Discipline)$/m,
    );
  ...
  it("matches every allowlisted authoring byte in stable order", () => {
    ...
    for (const path of PAYLOAD_PATHS) {
      expect(payloadBytes(path).equals(readFileSync(join(REPO_DIR, ...path.split("/"))))).toBe(
        true,
      );
