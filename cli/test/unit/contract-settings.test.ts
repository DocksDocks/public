...
describe("settings merge contract", () => {
  it("lets SoT win on conflict and keeps user-only keys", () => {
    const merged = mergeSettings({ theme: "kit", shared: "sot" }, { theme: "user", extra: "kept" }) as Record<
      string,
      Json
    >
  ...
  it("replaces non-permission arrays wholesale with the SoT array", () => {
    const merged = mergeSettings({ hooks: ["sot"] }, { hooks: ["user", "extra"] }) as Record<string, Json>
