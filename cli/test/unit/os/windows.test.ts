...
describe("Windows host OS", () => {
  ...
  it("double-escapes cmd metacharacters only for node_modules command shims", () => {
    ...
    expect(hostOs("windows").invoke("C:/project/bin/foo.cmd", ["a & b"])).toEqual({
      command: commandInterpreter,
      args: ["/d", "/v:off", "/s", "/c", String.raw`"C:\project\bin\foo.cmd ^"a^ ^&^ b^""`],
      windowsVerbatimArguments: true,
    });
  });
  ...
  it.each([
    ...
    [
      "SystemRoot when ComSpec is relative",
      { ComSpec: "cmd.exe", SystemRoot: "D:\\Windows" },
      "D:\\Windows\\System32\\cmd.exe",
    ],
    [
      "SystemRoot when ComSpec is empty",
      { ComSpec: "", SystemRoot: "D:\\Windows" },
      "D:\\Windows\\System32\\cmd.exe",
    ],
    [
      "SystemRoot when ComSpec is unset",
