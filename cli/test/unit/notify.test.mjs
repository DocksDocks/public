...
describe("Notification player selection", () => {
  it("prefers afplay on Darwin", () => {
    expect(
      selectPlayer({ platform: "darwin", sound: SOUND, which: whichFrom(["afplay", "ffplay"]) }),
    ).toEqual(["/usr/bin/afplay", SOUND]);
  });
  ...
  it("uses ffplay then paplay then aplay priority elsewhere", () => {
    expect(
      selectPlayer({
        platform: "linux",
        sound: SOUND,
        which: whichFrom(["ffplay", "paplay", "aplay"]),
      }),
    ).toEqual(["/usr/bin/ffplay", "-nodisp", "-autoexit", "-loglevel", "quiet", SOUND]);
    expect(
