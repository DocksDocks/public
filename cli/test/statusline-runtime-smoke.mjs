...
} from "../src/engine-native/claudeRuntime.ts";
...
function assertRun(label, run, expectedStdout) {
  ...
  if (run.stderr !== "")
    throw new Error(`${label} stderr was not empty: ${JSON.stringify(run.stderr)}`);
}
...
try {
  ...
  if (JSON.stringify(settings).includes("__DOCKS_KIT_"))
    throw new Error("materialized settings contain a sentinel");
  if (settings.hooks?.Stop !== undefined)
    throw new Error("materialized settings contain hooks.Stop");
  ...
  if (sessionRun.exitCode !== 0 || sessionRun.stderr !== "") {
    throw new Error(
      `SessionStart failed: exit=${sessionRun.exitCode} stderr=${JSON.stringify(sessionRun.stderr)}`,
    );
  }
  ...
  const notificationRun = record([notification.command, ...notification.args], "{}", {
    ...baseEnv,
    PATH: "",
  });
  ...
  if (directP95 > directCeiling) {
    throw new Error(
      `statusLine direct Bun p95 ${directP95.toFixed(2)}ms exceeds ${directCeiling}ms`,
    );
  }
  console.log(
    `runtime-smoke: direct Bun exact bytes OK; p95=${directP95.toFixed(2)}ms ceiling=${directCeiling}ms`,
  );
  if (mode === "posix") {
    const timings = [];
    for (let index = 0; index < 30; index += 1) {
    ...
    const ceiling = 250;
    if (median > ceiling)
      throw new Error(`statusLine posix median ${median.toFixed(2)}ms exceeds ${ceiling}ms`);
    console.log(
      `runtime-smoke: posix exact commands OK; median=${median.toFixed(2)}ms ceiling=${ceiling}ms`,
    );
  } else {
    ...
    if (!windowsCommand.startsWith(commandPrefix)) {
      throw new Error(
        `Windows statusLine command has an unexpected shape: ${JSON.stringify(windowsCommand)}`,
      );
    }
    ...
    if (powershell === null || powershell === "") {
      console.log(
        "runtime-smoke: windows encoded command shape OK; powershell.exe unavailable, execution timing skipped",
      );
    } else {
      const timings = [];
      for (let index = 0; index < 30; index += 1) {
