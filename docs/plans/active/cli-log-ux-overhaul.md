...
- [x] `bun run test:unit && bun run golden:dryrun && bun run golden:mutation` green; both `--prove-red` legs print `prove-red OK` and exit non-zero; goldens change only in commits that also change engine output, per tier (each output-changing tier = one reviewed commit with its golden diff).
