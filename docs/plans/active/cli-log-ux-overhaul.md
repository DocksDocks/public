...
| 6 | **Tier 2 — DependencyManager** (`cli/src/engine-native/deps.ts`): typed contract — `ToolId` union (git, claude, codex, rtk, bun, npx, agent-browser, chrome-for-testing, LSP binaries); `DependencySpec { id, requirement: "required" \| "optional", probe, versionCmd, installHint(platform) }`; `ProbeResult = present(version?) \| missing \| broken(reason)`. Required-missing stays a hint-bearing error preserving today's exit codes; optional-missing = exactly one deduplicated `[warn] <tool> not installed — <install command>` per run. `SoT/toolchain.json` remains the single source of version/pin policy — `toolchain.ts` keeps gate/ensure orchestration and consumes the manager for probe/hint; the duplicated git hint collapses to one definition. | 5 | done |
| 7 | **Tier 3 — Platform service** (`cli/src/engine-native/os.ts`): narrow capability seam — `platform()` (`linux/darwin/windows`), package-manager style for hints (winget/brew/apt-style wording only; per-tool package identifiers stay in `DependencySpec`), shell-rc applicability; symlink stays try-then-fallback (capability/error-driven, not platform-predicted). Scattered `process.platform` branches in sync modules route through it; pure path/exec helpers in `exec.ts` are exempt and listed in the policy spec. | 6 | done |
| 8 | **Effect integration at the rim**: define `Context.Tag`s + live `Layer.succeed` layers for Logger/DependencyManager/Platform built from one shared factory; compose once at `cli/src/main.ts` and hand sync interfaces through the `engine.ts` seam; `native-raw` and `engineCapture` call the same factory directly. Test layers demonstrated in one unit test per service. | 5, 6, 7 | done |
| 9 | **Verify + docs**: full suite green (`bun run test:unit`, `bun run golden:dryrun`, `bun run golden:mutation`, both prove-red legs `prove-red OK` + non-zero exit, `bun run typecheck`); manual smoke: first sync on a fresh fixture HOME shows changes + warns with hints, immediate re-run's default stderr shows warns/summary only; update `cli/docs/flags.md` + `cli/docs/overview.md`; CHANGELOG entry. | 5–8 | done |

## Acceptance criteria

- [x] Same-HOME replay golden case: second consecutive `sync` run at default verbosity emits **zero** stderr lines matching `already in sync|already initialized|already set|already opted in|already empty|up to date|\bpresent \(|LSP server binaries present|model already |left as-is` (extended after the implementation review — F9) (refined from the draft regex — a loose `already` false-positives on change lines embedding count phrasing like "(+1 new, 0 already present)"); the same run with `--verbose` emits them. Verify: the Step-4 replay case asserts both.
- [x] Channel contract enforced by tests: `status --json` stdout parses as JSON with verbose logging enabled; logger writes are stderr-only. Verify: Step-4 channel assertions fail if violated (prove: temporarily point one logger write at stdout → suite goes red; revert).
- [x] Missing git (via harness `maskTools: ["git"]`, separate Claude-only / Codex-only / combined-sync cases): exactly one deduplicated `[warn] git not installed — <platform-correct command>` per run in the combined case, sourced from `DependencyManager`; win32 hint covered by a unit test.
- [x] `grep -rn "winget install Git.Git" cli/src` → exactly one definition site.
- [x] `grep -rn "process.platform" cli/src/engine-native --include="*.ts"` → matches only in `os.ts` + the exec-helper exemptions named in the policy spec.
- [x] Public flags work end-to-end: `--verbose` and `-v` accepted on `sync`, `model`, `toolchain`; `DOCKS_KIT_VERBOSE=1` on the raw channel — each covered by a Step-4 integration case.
- [x] `bun run test:unit && bun run golden:dryrun && bun run golden:mutation` green; both `--prove-red` legs print `prove-red OK` and exit non-zero; goldens change only in commits that also change engine output, per tier — EXCEPT purely additive test-only rows (recording new cases, e.g. the Step-4 replay rows), which may land with the harness commit that introduces them (criterion revised at completion review, finding C5).
- [x] `bun run typecheck` green; services are `Context.Tag`s with named live + test layers; no `any` seams.
- [x] Dry-run stdout remains a complete inspection report (unfiltered) — dry-run golden diffs come only from wording changes named in the policy spec, not from filtering.
...
- Step 6 note (2026-07-09): deps.ts registry landed (ToolId, required/optional, probe, platform-parameterized installHint, dedup'd `warnMissing`); consumers: both git sites (dup collapsed to one definition — `grep -rn "winget install Git.Git" cli/src` = 1), claude/codex install hints, preflight jq/curl. Golden: 3 new git-masked cases; combined sync emits exactly ONE `[warn] git not installed — <hint>`; existing case outputs untouched. Unit tests cover win32/linux/darwin hints. bwrap's pm-detection chain intentionally stays in codexSync (probes the live system, richer than a static hint — Platform-service candidate in Step 7).
- Step 7 note (2026-07-09): `os.ts` landed as the single `process.platform` reader (`rawPlatform`, `platformName` linux/darwin/windows/unknown, `isWindows`, `isLinux`, `shellRcApplicable`). Migrated: codexSync `bwrapSupportedOs`, skillsSync ×4 (realpathEquals win32, symlink "dir" type, `--with-deps` linux, effect-solutions .exe shim), claudeSync ×2 (rtk win32 branch, setx connector env), toolchain.ts `platformOs` (via `platformName()`, unknown→""), deps.ts ×6 hint defaults (`pf = rawPlatform()`, still injectable). bwrap pm-detection chain stays in codexSync per Step-6 note (live-system probe, not platform prediction). Acceptance grep clean: `process.platform` matches only os.ts + exec.ts:27,45 — exemption named in DESIGN.md "### Platform seam"; module map refreshed (output.ts row → logger.ts/deps.ts/os.ts). Pure refactor verified: typecheck green, vitest 14/14, goldens 21+46 zero diffs, both prove-red legs red.
- Implementation cross-check (2026-07-09): [codex gpt-5.6-sol xhigh] reviewed commits 6c530d1^..f252d06 (steps 2,4,5a,5b,6,7) — 10 findings (1 high / 5 med / 4 low), 10 accepted, 0 rejected; all fixed in the same day's review-fix commit. F1 config.toml.bak-before-compare → staging-copy merge, .bak only on real change (replay goldens now show the preserved recovery copy); F2 unconditional next-steps → Ctx.nextStepTriggers (claudePlugins/claudeRestart/codexRestart/skillsRestart), advice prints only on trigger or --verbose (dry-run advice dropped — policy-covered); F3 "Installing…" change-before-success lines → verbose (chrome download line → warn: it announces a sudo prompt); F4 enableOptionalPlugin tracks marketplaceAdded/installed/enabled; F6 reassertEnabledState compares canonical forms before writing + reports, chmod repairs via exec.ensureExecutable feed changed, skill snapshot via writeFileIfChanged; F5 registry gained npm/npx (skillsSync npx gate + agent-browser/LSP npm warns route through warnMissing) — chrome-for-testing/LSP binaries stay OUT of the registry deliberately (kit-managed artifacts, not PATH tools; boundary noted); F7 probe returns broken(reason) on nonzero version probe; F8 git hints now executable per-platform (brew/apt wording); F9 NOOP_RE extended + acceptance regex synced; F10 public verbose legs now assert a verbose-only line reaches stderr for sync/model/toolchain × --verbose/-v (model legs ordered before sync replays — a flag-less re-merge would re-canonicalize settings), status --json and dry-run run under DOCKS_KIT_VERBOSE=1. Golden update intentional: 11 mutation + 21 dryrun case diffs (the 21 dry-run cases repeat four unique advice-line removals), all named above. Pre-existing follow-up (not in range): models.ts:52 writes catalog data to stderr.
- Completion cross-check ingest (2026-07-09): [codex gpt-5.6-sol xhigh] completion leg — 7 findings (2 high / 2 med / 3 low), verdict PARTIAL. Dispositions: C3 (Platform→hints) FIXED — `makeDependencyManager(platform)` derives hint defaults + warnMissing platform from the injected Platform, combined-graph unit test added (19/19); C4 FIXED — DESIGN.md now states model/toolchain have no summary block by design (their stdout is the deliverable); C5 FIXED — acceptance criterion revised to allow purely additive test-only golden rows; C6 FIXED — affected_paths gained exec.ts, engine-native/services.ts, cli/src/services.ts, engine.ts, main.ts; C7 FIXED — CHANGELOG count 46→47. C1/C2 (module-global logger emitters; toolchain probes not behind DependencyManager; Chrome/LSP ToolIds absent) — SCOPE DECISION by user via picker: seams-only stands; Goal sentence narrowed to match; full engine-internal DI scaffolded as follow-up plan `engine-full-di`. DISAGREEMENT: injected-services depth — [codex] Goal requires full engine DI, plan not shippable / [claude] Out-of-scope clause explicitly deferred the deep port; the rim + ctx.services IS the contracted deliverable. Kept: seams-only — decided by user via picker, because all 9 functional acceptance criteria pass and the deep port is a plan-sized change of its own.
- Follow-up cross-check (2026-07-09): [codex gpt-5.6-sol xhigh] reviewed f252d06..2e330cf (steps 8, 9 + fix batch) — 6 findings (3 med / 3 low), 6 accepted, 0 rejected; verdict on the prior batch: 8/10 fully closed, F2/F6 partial + one F4 edge, all closed now. R1 successful `plugin disable` now feeds reassert's changed result; R2 optional-plugin partial successes emit an accurate change line before the failure warn; R3 CHANGELOG dedup claim scoped to registry-routed tools; R4 Step-8 seam handoff actually implemented — `engine()` yields the three Tags and passes `{logger, deps, platform}` into `runEngineNative(argv, services)`, Ctx carries `services`, native-raw passes `makeEngineServices()`, and ctx-scoped call sites consume `ctx.services.deps`/`ctx.services.platform` (warnMissing ×5, install hints ×4, isWindows/name ×5) so PlatformTest/DependencyManagerTest injection is real below the rim; named residue of the seams-only scope: module-binding leaves in InstallFn callbacks (agentBrowserInstall isLinux) and `linkOrCopy` (no Ctx; capability-fallback site), plus the static hint default in deps.ts. R5 claudeModel change sets claudeRestart (Codex path already did) + new replay golden `sync claude` → `sync claude --claude-model=fable` proving restart advice from the model trigger alone (47 mutation cases now). R6 plan note corrected: 21 dry-run cases (4 unique advice shapes), not 4. Reviewer confirmed: all golden diffs named/intended, no unnamed behavior change, NOOP_RE no false positives, broken-probe verified live.
- Step 9 note (2026-07-09): full suite green — typecheck, vitest 17/17, goldens 21+46, both prove-red legs `prove-red OK` + exit 1. Manual smoke on a fresh fixture HOME (git-masked): first run 12 change lines + exactly ONE `[warn] git not installed — <hint>`; immediate replay's default stderr has ZERO no-op lines. Acceptance falsification run live: routing `warn` to stdout in logger.ts initially passed the suite — coverage gap (all invariant runs were fully-stubbed and warn-free) closed by a new git-masked split leg in `channelInvariantProblems` (warn must appear on stderr and not stdout); sabotaged suite now exits 1, reverted suite green. Docs: `--verbose`/`-v` row in cli/docs/flags.md, quiet-on-no-ops design rule in cli/docs/overview.md, CHANGELOG section added. All 9 acceptance criteria checked.
- Step 8 note (2026-07-09): rim landed — shared factory `cli/src/engine-native/services.ts` (`makeEngineServices`: logger from logger.ts bindings, `DependencyManager` over deps.ts, `Platform` via `makePlatform(pf?)` — pf injectable for tests); Effect side `cli/src/services.ts` with `Context.Tag`s (`docks-kit/Logger|DependencyManager|Platform`), `Layer.succeed` live layers off ONE factory instance, `EngineServicesLive = Layer.mergeAll(...)`, and named test-layer constructors (`LoggerTest(sinks)`/`PlatformTest(pf)`/`DependencyManagerTest(impl)`). Composed once at main.ts (`Layer.mergeAll(BunContext.layer, EngineServicesLive)`). Consumers: sync command's update nudge warn now goes through `yield* LoggerService` (R flows through @effect/cli to the single provide); `engineCapture`'s capture-failed warn calls the factory directly (child-spawn path outside the runtime) — both former hand-rolled ANSI writes eliminated, byte-identical output. native-raw uses the same module bindings the factory wraps. `cli/test/unit/services.test.ts`: one test per service test layer (verbose gating, win32/linux platform views, stub dep manager) — vitest 17/17. Goldens 21+46 zero diffs (public-CLI legs exercise the new composition), both prove-red legs red, typecheck green.
