...
| 2 | CI smoke on `windows-2025` (Git Bash): `bash -n` all shell, `bash lib/engine.sh sync --dry-run` under a temp HOME, `toolchain check` asserting bwrap absent + no unknown-OS warn. Guards the frozen escape hatch | 1 | in-flight (.github/workflows/windows-smoke.yml pushed; awaiting first green run) |
