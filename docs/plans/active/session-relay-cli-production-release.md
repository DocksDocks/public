...
updated: "2026-07-18T22:27:50-03:00"
started_at: null
assignee: null
...
RUNS_JSON="$(gh api --paginate --slurp 'repos/DocksDocks/public/actions/workflows/release-cli.yml/runs?per_page=100')"
jq -e '([.[].workflow_runs[] | select(.event == "push" and .head_branch == "cli-v0.9.0")] | length) == 0' <<<"$RUNS_JSON"
NPM_READ_CACHE="$PUBLIC_PREFLIGHT_DIR/npm-cache"
mkdir -m 700 "$NPM_READ_CACHE"
test "$(stat -c '%a' "$NPM_READ_CACHE")" = 700
NPM_VERSIONS="$(NPM_CONFIG_CACHE="$NPM_READ_CACHE" npm view docks-kit versions --json)"
NPM_VERSIONS="$NPM_VERSIONS" node --input-type=module -e 'import assert from "node:assert/strict"; const versions=JSON.parse(process.env.NPM_VERSIONS); assert.ok(Array.isArray(versions)); assert.ok(!versions.includes("0.9.0"));'
```
...
for attempt in $(seq 1 60); do
  RUNS_JSON="$(gh api --paginate --slurp 'repos/DocksDocks/public/actions/workflows/release-cli.yml/runs?per_page=100')"
  RUN_COUNT="$(jq '[.[].workflow_runs[] | select(.event == "push" and .head_branch == "cli-v0.9.0")] | length' <<<"$RUNS_JSON")"
  test "$RUN_COUNT" -le 1
  if test "$RUN_COUNT" -eq 1; then
    RUN_ID="$(jq -r '[.[].workflow_runs[] | select(.event == "push" and .head_branch == "cli-v0.9.0")][0].id' <<<"$RUNS_JSON")"
    break
  fi
...
| A8 | `git diff --check` | Exit 0; release-preparation changes have no whitespace errors. |
...
| P2 | `PUBLIC_RELEASE_COMMIT="$(PLAN_PATH=docs/plans/active/session-relay-cli-production-release.md node --input-type=module -e 'import fs from "node:fs"; import assert from "node:assert/strict"; const prefix="Completion-review-receipt: "; const lines=fs.readFileSync(process.env.PLAN_PATH,"utf8").split(/\r?\n/).filter((line)=>line.startsWith(prefix)); assert.equal(lines.length,1); const receipt=JSON.parse(lines[0].slice(prefix.length)); assert.equal(receipt.schema,5); assert.equal(receipt.phase,"completion"); assert.equal(receipt.completion_verdict,"passed"); assert.match(receipt.reviewed_head,/^[0-9a-f]{40}$/); process.stdout.write(receipt.reviewed_head);')" && RUNS_JSON="$(gh api --paginate --slurp 'repos/DocksDocks/public/actions/workflows/release-cli.yml/runs?per_page=100')" && RUNS_JSON="$RUNS_JSON" PUBLIC_RELEASE_COMMIT="$PUBLIC_RELEASE_COMMIT" node --input-type=module -e 'import assert from "node:assert/strict"; const pages=JSON.parse(process.env.RUNS_JSON); const runs=pages.flatMap((page)=>page.workflow_runs).filter((run)=>run.event==="push"&&run.head_branch==="cli-v0.9.0"); assert.equal(runs.length,1); assert.equal(runs[0].head_sha,process.env.PUBLIC_RELEASE_COMMIT); assert.equal(runs[0].status,"completed"); assert.equal(runs[0].conclusion,"success");'` | Exit 0; this command re-derives the receipt-bound identity and exhaustively paginates the pinned workflow without a server-side search cap; exactly one matching push run exists, targets `PUBLIC_RELEASE_COMMIT`, is completed, and concluded success. |
