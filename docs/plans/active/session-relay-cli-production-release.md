...
for attempt in $(seq 1 60); do
  RUNS_JSON="$(gh run list --repo DocksDocks/public --workflow release-cli.yml --event push --limit 100 --json databaseId,headBranch)"
  RUN_COUNT="$(jq '[.[] | select(.headBranch == "cli-v0.9.0")] | length' <<<"$RUNS_JSON")"
  test "$RUN_COUNT" -le 1
  if test "$RUN_COUNT" -eq 1; then
    RUN_ID="$(jq -r '[.[] | select(.headBranch == "cli-v0.9.0")][0].databaseId' <<<"$RUNS_JSON")"
    break
  fi
