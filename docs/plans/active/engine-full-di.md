...
updated: "2026-07-09T21:39:41-03:00"
...
- [ ] Temp-dir leak closed (Step 7): run both golden suites AND both `--prove-red` legs, then `ls /tmp | grep -c '^golden-'` → `0`; `git diff` for the Step-7 slice touches only `cli/test/lib/harness.ts` (+ this plan file); goldens byte-identical.
