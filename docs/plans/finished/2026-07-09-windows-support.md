...
| 5 | Port modules behind `cli/src/engine-native/`, one PR-sized commit each, gated by 4(a)+4(b): (a) scaffold + `DOCKS_KIT_ENGINE` selector + settings merge, (b) claude.json + connector env (Windows: `setx`), (c) codex TOML merge + rules/agents-md, (d) plugin passes, (e) skills + toolchain + removals + modifiers + direct modes. Bash stays the DEFAULT throughout; `DOCKS_KIT_ENGINE=native` opts in. Unit layer per the Effect standard (user decision 2026-07-08): vitest 3.2.7 + `@effect/vitest` 0.29.0 (exact pins), `it.effect` suites + jq-differential oracles in `cli/test/unit/`, run by parity.yml; the parity harnesses stay plain black-box scripts (they compare two child processes, not Effect code) | 4 | done — 5(a) 3ba8b26, 5(b) 6ab58f6, 5(c) d012df8, 5(d) d133880, 5(e) sync claude (final). Gate: FULL dry-run (21 pairs) + FULL mutation (17 rows + 7 TOML x2) parity green with `--native` on Linux, both prove-reds red; native CI legs added to parity.yml (Linux-only — Windows native runs move to step 7: the harness's bash-script stubs aren't spawnable by a native win32 process; solved with the PowerShell CI there). Post-port gpt-5.5 review round (3 findings, all fixed): selector split into public `native` (routed at the engine.ts seam AFTER @effect/cli parses) vs harness-private `native-raw` (main.ts raw bypass); rtk init failure now aborts like bash `set -e`; 6 non-happy-path stub-override matrix rows (rtk-init-fails, agent-browser stale / missing+npm-install-fails / above-verified±`--yes` / npm-offline) → 23 matrix rows total, all green bash-vs-native |
| 6 | Flip: `engine.ts` defaults to EngineNative on all platforms (`DOCKS_KIT_ENGINE=bash` opt-out). The flip is ONE default change at the `nativeSelected()` seam in `cli/src/engine.ts` — never in `main.ts`, whose `native-raw` bypass is harness-only. Real `sync claude` + `model` round-trip verified on this Linux machine against a settings backup; mutation-parity suite green is a hard precondition | 5 | done — one-line flip at the seam. Evidence: bash-probe (PATH-shadowed `bash` marker) shows 0 bash spawns on default run, ≥1 with `DOCKS_KIT_ENGINE=bash`; real round-trip on this machine: `model claude sonnet` → deployed `.model=sonnet` → flag-less `sync claude` reverts to `opus`; post-round-trip settings.json byte-identical to pre-backup (jq -S diff empty); `.claude.json` diff = Claude Code's own usage counters + sync re-adding `showTurnDuration` (documented behavior); engine `.bak` vs current = exactly the model revert |
| 7 | Windows-native CI on `windows-2025` under PowerShell with `HOME` unset: `$env:DOCKS_KIT_ENGINE="native"; bun cli/src/main.ts sync --dry-run`, `status --json`, `plugins list`, `model claude` (get), `toolchain check` — exact commands in the workflow; runnable pre-flip via the selector | 5 | done (6b04248, green run 28980202087, first attempt) — `native-windows` job in parity.yml: hermetic `%USERPROFILE%` home seeded from SoT, HOME unset; all five commands + the toolchain ensure gate branches (non-TTY pinned fallback + `--yes`) driven by an npm.cmd stub reporting latest above verified. The npm steps double as the `.cmd` spawn canary — green proves bun 1.3.14's spawnSync launches `.cmd` on Windows (npm row shows its version; gate routed via npm.cmd, not the offline path). Scope decision: stub-based parity legs stay POSIX-only (shebang stubs aren't win32-spawnable; porting them to .exe tests the stubs, not the engine) — Windows EngineNative coverage is this job + step 9 |
| 8 | Windows entrypoints: document + verify the supported Windows command surfaces — compiled `docks-kit-windows-x64.exe` (release asset; THE supported no-toolchain path), `bun add -g docks-kit` shim behavior on Windows (verify bun creates a working shim for the `#!/usr/bin/env bun` bin), `install.sh` explicitly documented Unix-only; README + platforms topic updated | 5 | done (114e2c9, windows-entrypoints.yml green run 28980967423, first attempt) — exe-smoke: cross-compiled .exe runs `sync --dry-run` + `status` on windows-2025 with NO setup-bun against a hermetic seeded `%USERPROFILE%`; bun-shim: `bun add -g` from a packed tarball, `models claude` + `toolchain check` from a foreign cwd (bundled-SoT kit-home). Prerequisite fix in the same commit: `engineCapture` in a bun-compiled binary re-spawned itself with the main.ts path as a stray engine arg ("Unknown arg", found with the linux-x64 binary locally) — compiled mode detected via the virtual entry path (`/$bunfs/` POSIX, `~BUN` Windows), re-spawn passes args only. install.md gained the Windows-entrypoints section + Unix-only marking; platforms.md rewritten post-flip; README row updated. The exe needs a kit checkout or DOCKS_KIT_HOME (SoT isn't embedded — the exe replaces the toolchain, not the config repo). gpt-5.5 review round (1 MAJOR + 2 MINOR, all fixed in 11aac77, both workflows green run 28981804635/28981804644): exe-status smoke asserts a real npm toolchain row (was vacuous — engineCapture returns "" on a failed child, which now also warns to stderr); compiled detection anchored to the drive-rooted `~BUN` segment (bare `.includes` could false-positive on a ~BUN-named checkout dir); `SoT/**` added to the workflow's path filters (the bun-shim job proves bundled-SoT manifest reads) |
| 9 | Real-machine interactive verify (user's own Windows machine, manual): `.exe` sync from native PowerShell; `%USERPROFILE%\.claude` loaded by Claude Code; rtk hook fires (`rtk gain` shows activity); each deployed hook/statusline command executed-or-intentionally-skipped and the outcome recorded here | 6,7,8 | done (2026-07-09, user machine, via the `bun add -g` npm path 0.1.1→0.1.5) — full sync + 4 `docks-kit update` cycles green; bundled-SoT kit-home; setx connector env (set + idempotent skip); LSP servers via npm; universal skills + agent-browser (Chrome for Testing download) work on Windows; plugin passes green with git (docks marketplace +1, +3 plugins; official-marketplace plugins needed first `claude` login, then installed); Claude Code 2.1.202 (winget) loads the deployed config — status line renders, model opus. **5 real bugs found + fixed + released during the verify** (0.1.2–0.1.5): Unix-only jq hint; connector env went to .bashrc instead of setx (kill switch never applied — now CI-asserted); effect-solutions .exe shim false-negative; EFAULT crash healing the skills junction (sync-aborting); 8 opaque plugin failures with git absent (now one loud per-OS guard + toolchain `git` row). Intentionally skipped: rtk hook + notification sound (rtk/ffplay not installed on that box — engine warns correctly; retest anytime); `.exe` interactive run (surface CI-verified in windows-entrypoints; npm path chosen by user) |
| 10 | Docs + skills: platforms/README to "supported"; the five kit-mechanic bash skills get a feature-freeze banner (not deletion); new engine-native skill or references file | 6 | done — platforms.md/README updated in the step-8 commit (Windows "supported, pending one manual gate" = step 9); all SIX kit-mechanic bash skills (the plan predates toolchain-context) carry the feature-freeze banner; sync-orchestration-context's dispatch chain rewritten for the post-flip seam (EngineNative default, three-place flag rule + parity gate); new `engine-native-context` skill covers the parity contract, p() rule, ExitError set-e twin, selector geometry incl. compiled re-spawn, jq/awk semantic quirks, harness machinery, Windows specifics |
...
- Step 5 (each module): dry-run AND mutation parity green on all fixtures —
  native-vs-bash on linux (done); the windows-2025 native leg moved to step 7,
  where the stub-spawning + PowerShell work lands (bash self-parity already
  runs green on windows-2025).
...
- **2026-07-08T18:20:00-03:00**: `DOCKS_KIT_ENGINE=native` originally
  short-circuited in `main.ts` BEFORE @effect/cli parsed → live regression
  for anyone exporting the env: pickers, `--flag value` space forms, and
  non-engine commands (`status`, `models`, `docs`) all broke (gpt-5.5 review
  finding 1) → the public selector must route AFTER the CLI layer, at the
  `engine()`/`engineCapture()` seam; a raw pre-parse bypass is only valid as
  a harness-private channel (`native-raw`).
- **2026-07-08T18:20:00-03:00**: EngineNative ignored `rtk init --global`'s
  exit status and logged success where bash's `set -e` aborts the sync
  (gpt-5.5 review finding 2) → when porting bash, every unguarded command is
  an implicit abort point; the port must throw (`ExitError`) wherever bash
  would die. Caught nothing in parity because the happy-path stubs never
  failed — which was review finding 3, fixed with per-row stub overrides.
