...
- 2026-07-10 post-ship CI closure: merge `50b99db` ran the deferred-to-CI Windows legs for real. `golden-regression` passed; `windows-entrypoints` failed only inside the new bun-shim assertion because bun 1.3.14 on Windows prints the blocked package as `.\node_modules\@parcel\watcher @2.5.6` (backslashes) and the parser matched only the forward-slash spelling — the blocked-set contract itself held (exactly one blocked package, correct script line). Fixed separator-agnostically on main in `da15014`; `windows-entrypoints` then passed, closing the deferred item with real-runner evidence.
