...
updated: "2026-07-10T10:24:20-03:00"
...
- [x] Deployed `~/.claude/settings.json` contains no sentinel; template cardinalities are Bun=2, notify=1, SessionStart=1, statusline=1; SessionStart and Notification each have one handler; hook commands use an absolute real `bun`/`bun.exe` plus one-element `args`; Windows ignores a shadowing `bun.cmd`; statusline uses the POSIX or decoded PowerShell missing-file guard and retains `refreshInterval: 5`. The exact stored command executes paths containing spaces/apostrophes under POSIX shell, Windows Git Bash, and native PowerShell CI with identical expected channels/bytes. (Closed by PR #9 CI at 2ceca72: native-windows job 86338451271 — `powershell exact commands OK` and `git-bash exact commands OK`, byte-exact channels for all 30 runs each.)
...
- [~] Payload generator inventory is exact: the three `.mjs` paths are embedded, the three `.sh` paths are absent, `bun cli/scripts/generate-sot-payload.ts --check` exits 0, and a packed/compiled CLI syncs the scripts from a foreign cwd without publishing `SoT/` files. (Closed by PR #9 CI at 2ceca72: windows-entrypoints run 29085602391 — exe-smoke compiled deferred-migration/foreign-cwd legs and bun-shim tarball/foreign-cwd/sync/direct-hook-execution legs all pass.)
...
- [x] Every slice and final handoff pass typecheck, all Vitest tests, both golden suites, both prove-red legs, and generated-payload freshness; native Windows workflow executes the real materialized commands rather than only inspecting JSON shapes. (Closed by PR #9 CI at 2ceca72: all five checks green — golden, native-windows, build-exe, exe-smoke, bun-shim; native-windows executed the stored commands under real PowerShell and Git Bash.)
...
- 2026-07-10 (post-handoff, orchestrator): branch pushed as PR #9 to run the Windows CI legs. Two CI-infra commits by the orchestrator: 6059844 (pin bun global bin pre-USERPROFILE-flip — superseded diagnosis) and 327e31f (`.gitattributes` `* text=auto eol=lf`: payload `--check` was structurally stale on CRLF Windows checkouts — an inherited main regression from the embedded-payload merge, main's last two windows-entrypoints runs were already red; plus pack-step guard and a smoke warmup later removed). After these, bun-shim/golden/exe-smoke/build-exe green; native-windows red on statusline stderr.
- 2026-07-10 fix round 1: Cross-check (2026-07-10): [codex gpt-5.6-sol xhigh] fresh-context implementation review at 327e31f — 5 findings (3 med, 2 low), NOT READY; [claude] independently verified all 5 against source and CI logs before accepting; 5 accepted, 0 rejected. Fixes applied by the orchestrator directly (worker blocked by Codex usage limit until 09:45 local; queued fix-round mail superseded by stand-down notice): (1) `$ProgressPreference = 'SilentlyContinue'` prefixed inside the encoded PowerShell statusline guard — CLIXML progress records recurred across runs, so the stored command is now byte-clean itself and the unasserted smoke warmup was removed; (2) `pruneJsonKeys` presence is own-property existence, so `"Stop": null` is pruned (new ready-migration test); (3) POSIX Bun resolution now requires absolute executables from PATH and BUN_INSTALL fallbacks (new relative-path test); (4) `formatTokensK` routes fractional-million tenths through `roundHalfEven` (1250k → 1.2M, test corrected from the wrongly pinned 1.3M). Golden ledger amendment (authorized by this fix round): 15 mutation labels changed — each solely the `.claude/bin/statusline.mjs` sha256 (30 diff lines audited, all statusline.mjs; canaries untouched); dry-run 22 unchanged. Post-fix gates: tsc 0; Vitest 97/97; dry-run OK 22; mutation OK 52; both prove-red exit 1 "prove-red OK"; payload `--check` 0; POSIX smoke p95 34.85/40.28 ms. Scope audit refreshed: 63 changed paths, 61 declared entries, zero uncovered (`.gitattributes` added).
- 2026-07-10 (orchestrator): after fix round 1, native-windows exposed one more workflow gap — `model claude (get)` had no deployed settings once the raw-template `Copy-Item` was (correctly) removed; 2ceca72 adds a real `sync claude --skip-rtk` step in the hermetic home first (also giving parity.yml a native-Windows ready-cutover leg; reproduced and sequence-verified locally). PR #9 CI then went fully green at 2ceca72: golden, native-windows (PowerShell p95 657.33 ms/750, Git Bash 552.60 ms/750, direct Bun 125.85 ms/200, byte-exact channels), build-exe, exe-smoke, bun-shim. All four previously-partial acceptance items closed with run links above.
- 2026-07-10 (post-ship, user-requested): the outer-shell latency gate in `statusline-runtime-smoke.mjs` flaked once on the ship-commit run (Git Bash p95 1025.56 ms vs 750 on code identical to the green 2ceca72 run — runner load, cleared on rerun). The outer-shell legs now assert the **median** of the 25 measured runs against the same 250/750 ms ceilings (p95 of 25 samples is the second-worst run and tracks runner noise, not the command); the direct-Bun leg keeps its p95 ≤100/200 ms contract, and the byte-exact stdout/empty-stderr assertions on all 30 runs are unchanged.
