...
- Extract Claude-specific bootstrap into `lib/claude.sh`: RTK install/upgrade (the download-then-run installer + version check), the six idempotent plugin-install passes via `claude plugin` CLI, `showTurnDuration` → `~/.claude.json` patch, hook script copy from `ssot/.claude/hooks/` → `~/.claude/hooks/`, status line script copy.
- Refactor `sync.sh` to thin orchestrator (target ≤50 lines): `source lib/common.sh`; detect `ssot/.claude/` and `ssot/.codex/`; for each detected SSOT, `source lib/<tool>.sh` and call its public entry point (`claude::sync`, `codex::sync`).
- Add `ssot/.codex/` SSOT scaffolding: minimal initial set (placeholder `AGENTS.md`, `config.toml` skeleton, `agents/` placeholder, `.gitkeep`). Real Codex config content is out of scope — this plan establishes the directory structure only.
...
- Update root `AGENTS.md`: remove "(planned)" qualifiers on `lib/`, `CODEX.md`, `ssot/.codex/` rows once they exist.
...
- [ ] `./sync.sh` with only `ssot/.claude/` present skips Codex entirely (no `lib/codex.sh` sourced)
...
- [ ] `AGENTS.md` updated: the four "(planned)" markers (`lib/`, `CODEX.md`, `ssot/.codex/`, and `lib/<tool>.sh` references) all dropped
...
- **Detect-if-exists pattern over `--codex` flag** — user-confirmed in the session that landed the AGENTS.md bridge (2026-05-11). Rationale: SSOT layout is already declarative; flags add cognitive load and bias the user toward thinking the tools are equal-priority rather than "whatever you put in `ssot/` gets deployed."
- **SOLID alignment** — `lib/<tool>.sh` files satisfy SRP (one tool's lifecycle each). `lib/common.sh` is the shared abstraction layer. Adding a future tool means dropping in `lib/<newtool>.sh` and an `ssot/.newtool/` directory; `sync.sh` itself does not change.
